...
namespace ConsoleApp1
{
	class Program
	{
		...
		public static List<Book> setCartData()//this function creates a Cart List which is used to create a Book[] Cart
		{
		...
		public static void CheckCardNum(string CardID, ref bool ValidCard, ref string CardType)
		{
		...
		public static void CheckZipCode(ShippingInfo SI, ref bool ValidZC, ref bool ValidRg)
		{
			//normalize zip code
		...
		public static double BooksCost(Book[] Cart)
		{
			double totalPrice = 0;
			int copies = 0;
			int disc = 0;
		...
		public static double ShippingCost(ShippingInfo SI)
		{
		...
		{// the main function of our program is not tested!!!
			...
			ShippingInfo SI = new ShippingInfo("tzabela 11", "17124", "ATTIKHS", "GREECE");

			//1.
			bool v_isbn = ValidISBN("0441013597");
			//Console.WriteLine("0441013597 is valid? {0}", v_isbn);

			foreach(DataRow r in BooksStock.Rows)
            {
				string test = (string)r[0];
				Console.WriteLine("{0} is valid? {1}", r[0], ValidISBN(test));
            }
			/*
			//2.
			string CardID = "4027426132264440138";
			bool ValidCard = false;
			string CardType = "";
			CheckCardNum(CardID, ref ValidCard, ref CardType);
			Console.WriteLine("4027426132264440138 is valid? {0} and is a {1}", ValidCard, CardType);

			//3.
			bool validZC = true;
			bool validRG = false;
			CheckZipCode(SI, ref validZC, ref validRG);
			Console.WriteLine("{0}, {1}, {2} is valid? {3} , {4}", SI.Address, SI.Region, SI.Country, validZC, validRG);

			//4.
			int Stock = 0;
			bool is_av = IsAvailable(BooksStock, "0441013597", 3, ref Stock);
			Console.WriteLine("book 0441013597 has {0} and is available? {1}", Stock, is_av);

			//5.
			double cost = BooksCost(Cart);
			Console.WriteLine("the cost of your cart is {0}", cost);

			//6.
			double scost = ShippingCost(SI);
			Console.WriteLine("shipping Cost is : {0}", scost);

			//7.
			bool check;//IsAvailable(BooksStock, "9780544003415", 19, ref Stock);
			check = ProceedToBuy(BooksStock, Cart);
			Console.WriteLine("proceed to buy? {0}", check);
			//8.
			UpdateCartBasedOnAvailability(BooksStock, ref Cart);*/
		}
	}
